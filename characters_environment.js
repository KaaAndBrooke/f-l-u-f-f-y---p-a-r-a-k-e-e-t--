...
function game() {
  ...
  if (gameConfig.status === 'start') {
    ...
    textSize(40);
    textAlign(CENTER);
    text("Move Nose to Start and Play ", gameConfig.screenX / 2, gameConfig.screenY / 2);
    textSize(40);
...
function initializeCharacterStatus(character) {
  ...
  character.scale = 0.35;
  character["killing"] = 0; //while is killing enemy //god that's violent
  character["kills"] = 0;
  character["live"] = true;
...
function instializeInDraw() {
  ...
  //while killing //that's still violent
  if (mario.killing > 0) {
    mario.killing -= 1;
  } else {
    mario.killing = 0;
...
//nice, there was no need for the stars, but thanks
...
function StepOnEnemy(obj1, obj2) {
  ...
  if (obj1_Right >= obj2_Left && obj1_Left <= obj2_Right && obj1_Down <= obj2_Up + 7 && obj1_Down >= obj2_Up - 7 && obj2.live == true && obj2.touching.top) {
    obj2.live = false;
    obj1.killing = 30; //that's still extremely violent
    mario_kick.play();
    obj1.kills++;
...
// make character die if he touched by enemy //i f   h e   t o u c h e d .
